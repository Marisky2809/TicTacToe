...
public class NetworkClient
{
    ...
    private void Remote_ReceivedMessage(IAsyncResult ar)
    {
        ...
        {
            var bytesReceived = _client.GetStream()
                                       .EndRead(ar);
            if (bytesReceived > 0)
            {
                if (_store != null)
    ...
    private void ReadBuffer(int bytesReceived)
    {
        ...
        if (match.Success)
        {
            ...
            var lengthStr = text.Substring(startOfLength,
                                 endOfLine - startOfLength);
            var length = int.Parse(lengthStr);
            var payloadSoFar = text.Substring(endOfLine + 1,
                                          text.Length - (endOfLine + 1));
            var payload = Encoding.ASCII
                                  .GetBytes(payloadSoFar);
            if (payloadSoFar.Length == length)
            {
                var args = new
                           MessageReceivedEventArgs(payload,
                                             payload.Length);
