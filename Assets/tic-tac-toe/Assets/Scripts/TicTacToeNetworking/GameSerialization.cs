...
{
    public static byte[] CreateMove(int playerId,
                                    int cellIndex)
    {
        return CreateMessage(playerId,
    ...
    public static byte[] CreatePodium(int currentPlayer,
                                      int[] boardState)
    {
        return CreateMessage(currentPlayer,
                             MessageType.ServerShowPodium,
                             boardState);
    }
    public static byte[] CreateMessage(int playerId,
                                      MessageType type,
                                      int[] boardState = null)
    {
        var state = boardState == null ||
                    boardState.Length == 0 ? new int[1] { 0 }
                                           : boardState;
    ...
    public static GameMessage FromBytes(byte[] message)
    {
        ...
        var messageType = (MessageType)Enum.Parse(typeof(MessageType),
                        split[0]);
